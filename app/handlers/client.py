...
async def cancel_client_handlers(message: types.Message, state: FSMContext):
    ...
    if current_state is None:
        return
    await state.finish()
    await message.reply('Оформлення скасовано', reply_markup=start_menu)
...
async def tattoo_colors(callback: types.CallbackQuery):
    ...
    all_color = session.query(Pigments).filter_by(company_creator=callback_data[1]).filter_by(direction="Тату").all()
...
async def add_to_basket(callback: types.CallbackQuery):
    ...
    try:
        ...
        session.commit()
        await callback.answer(text='Пігмент додано')
                ...
                for pigment in pigments:
                    if pigment.id == item:
                        await bot.send_photo(message.from_user.id, pigment.photo, f'Назва: {pigment.pigment_name}\n'
                                                                                  f'об\'єм та ціни: {pigment.volume_and_price}',
...
def register_handlers_client(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(tatoo_creators, Text(startswith="Тату-пігменти_"))
    ...
    dp.register_callback_query_handler(permanent_zones, Text(startswith="Перманент-виробник"))
